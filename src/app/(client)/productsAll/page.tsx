...
type PriceRangeType = "all" | "under-20k" | "20k-50k" | "50k-80k" | "over-80k";
      ...
      <div className="space-y-2">
        ...
        <label className="flex items-center cursor-pointer">
          <input
            type="radio"
            name="price"
            checked={priceRange === "under-20k"}
            onChange={() => setPriceRange("under-20k")}
            className="w-4 h-4 text-blue-600"
          />
          <span className="ml-2 text-gray-700">Dưới 20.000đ</span>
        </label>
        <label className="flex items-center cursor-pointer">
          <input
            type="radio"
            name="price"
            checked={priceRange === "20k-50k"}
            onChange={() => setPriceRange("20k-50k")}
            className="w-4 h-4 text-blue-600"
          />
          <span className="ml-2 text-gray-700">20.000đ - 50.000đ</span>
        </label>
        <label className="flex items-center cursor-pointer">
          <input
            type="radio"
            name="price"
            checked={priceRange === "50k-80k"}
            onChange={() => setPriceRange("50k-80k")}
            className="w-4 h-4 text-blue-600"
          />
          <span className="ml-2 text-gray-700">50.000đ - 80.000đ</span>
        </label>
        <label className="flex items-center cursor-pointer">
          <input
            type="radio"
            name="price"
            checked={priceRange === "over-80k"}
            onChange={() => setPriceRange("over-80k")}
            className="w-4 h-4 text-blue-600"
          />
          <span className="ml-2 text-gray-700">Trên 80.000đ</span>
        </label>
      </div>
    </div>
  ...
  const filterAndSortProducts = (): void => {
    ...
    if (priceRange !== "all") {
      filtered = filtered.filter((product) => {
        if (priceRange === "under-20k") return product.price < 20000;
        if (priceRange === "20k-50k") return product.price >= 20000 && product.price < 50000;
        if (priceRange === "50k-80k") return product.price >= 50000 && product.price < 80000;
        if (priceRange === "over-80k") return product.price >= 80000;
        return true;
      });
    }
