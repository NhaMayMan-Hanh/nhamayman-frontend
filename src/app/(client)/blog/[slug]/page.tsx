...
import type { Comment, ExtendedBlog } from "../type";
...
export default function BlogDetailPage() {
   ...
   const hasLiked =
      user && blog?.likedBy?.some((id) => id.toString() === user.id.toString());

   useEffect(() => {
      const fetchBlog = async () => {
         try {
            const result = await apiRequest.get<ApiResponse<ExtendedBlog>>(
               `/client/blogs/${slug}`,
               {
                  noAuth: true,
               }
                     ...
                     {user ? (
                        ...
                        <div className="mb-8 bg-gray-50 rounded-lg p-6 text-center">
                           <p className="text-gray-600">
                              Đăng nhập để bình luận nhé!
                           </p>
                           <button className="mt-3 px-6 py-2 bg-pink-500 text-white rounded-lg hover:bg-pink-600">
                              Đăng nhập ngay
                           </button>
                        </div>
                     )}
