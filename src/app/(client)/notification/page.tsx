      ...
      <div className="bg-linear-to-br from-gray-50 to-gray-100 rounded-xl p-4 mb-3 text-sm border border-gray-200">
          ...
          <>
            {metadata.feedbackMessage && (
              <p className="text-gray-600 mb-2">
                <span className="font-semibold text-gray-700">Tin nhắn của bạn:</span>{" "}
                {metadata.feedbackMessage}
              </p>
            )}
            {metadata.adminResponse && (
              <p className="text-gray-700">
                <span className="font-semibold text-blue-600">Phản hồi:</span>{" "}
                {metadata.adminResponse}
              </p>
            )}
          </>
          ...
          <div className="space-y-2">
            {metadata.previousStatus && (
              <p className="text-gray-600">
                <span className="font-semibold text-gray-700">Trạng thái cũ:</span>{" "}
                <span className="capitalize">{metadata.previousStatus}</span>
              </p>
            )}
            {metadata.orderStatus && (
              <p className="text-gray-700">
                <span className="font-semibold text-green-600">Trạng thái mới:</span>{" "}
                <span className="capitalize font-medium">{metadata.orderStatus}</span>
              </p>
            )}
            {metadata.trackingNumber && (
              <p className="text-gray-700">
                <span className="font-semibold text-purple-600">Mã vận đơn:</span>{" "}
                <code className="bg-white px-2 py-1 rounded border">{metadata.trackingNumber}</code>
              </p>
            )}
            {metadata.cancelReason && (
              <p className="text-red-600">
                <span className="font-semibold">Lý do hủy:</span> {metadata.cancelReason}
              </p>
            )}
            {metadata.cancelledBy && (
              <p className="text-gray-600 text-xs">
                Hủy bởi: <span className="font-medium">{metadata.cancelledBy}</span>
              </p>
            )}
            {metadata.updatedBy && (
              <p className="text-gray-600 text-xs">
                Cập nhật bởi: <span className="font-medium">{metadata.updatedBy}</span>
              </p>
            )}
          </div>
        )}
        {type === "PROMOTION" && (
          <>
            ...
            {metadata.validUntil && (
              <p className="text-gray-600 text-xs">⏰ Có hiệu lực đến: {metadata.validUntil}</p>
            )}
          </>
        )}
      </div>
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg p-12 text-center border border-gray-100">
                  ...
                  <p className="text-gray-900 font-medium mb-2">Không có thông báo</p>
