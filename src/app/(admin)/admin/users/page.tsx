...
const filterUsers = (users: User[], searchTerm: string, filters: FilterOptions): User[] => {
    ...
    >
      ...
      >
        <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 flex items-center justify-between rounded-t-lg">
          <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Xác nhận xóa</h3>
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // @ts-ignore
        window.updateToast(toastId, errorData.message || "Xóa thất bại", "error");
      }
    } catch (err) {
      // @ts-ignore
                  ...
                  paginatedUsers.map((user) => (
                    <tr key={user._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        ...
                        <div className="md:hidden text-xs text-gray-600 mt-1">{user.email}</div>
