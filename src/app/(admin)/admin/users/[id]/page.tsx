...
export default function UserDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/users/${user._id}`, {
        method: "DELETE",
        credentials: "include",
      });

      if (res.ok) {
        updateToast(toastId, `Đã xóa người dùng "${user.name}" thành công!`, "success");
        setTimeout(() => {
          router.push("/admin/users");
        }, 1500);
      } else {
        const errorData = await res.json();
        updateToast(toastId, errorData.message || "Xóa người dùng thất bại", "error");
      }
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="bg-red-50 border border-red-200 rounded-lg p-8 max-w-md text-center">
          ...
          <p className="text-red-700 mt-2">{error || "Người dùng không tồn tại"}</p>
          <Link
            href="/admin/users"
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div>
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Chi tiết người dùng</h1>
            <p className="text-gray-600 mt-1">Thông tin đầy đủ về tài khoản</p>
          </div>
          <Link
            href="/admin/users"
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-sm p-6 text-center">
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                >
                  {user.isVerified ? "Đã xác thực email" : "Chưa xác thực email"}
                </p>
              </div>
            </div>
          </div>
