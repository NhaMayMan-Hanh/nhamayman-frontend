...
export default function EditUserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentAdmin && user._id === currentAdmin.id && formData.role !== user.role) {
  ...
  const handleSendPasswordReset = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/users/${userId}/send-reset-password`,
        {
          method: "POST",
          credentials: "include",
        }
      ...
      if (res.ok) {
        updateToast(toastId, `Đã gửi link đặt lại mật khẩu đến ${user.email}`, "success");
      } else {
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div>
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Chỉnh sửa người dùng</h1>
        ...
        {isEditingSelf && (
          <div className="mb-6 bg-amber-50 border border-amber-200 text-amber-800 px-5 py-4 rounded-lg flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold">Bạn đang chỉnh sửa tài khoản của chính mình</p>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Email chỉ người dùng tự thay đổi được</p>
            </div>
            ...
            <div>
              ...
              {isEditingSelf && (
                <p className="text-xs text-amber-600 mt-1">Không thể tự thay đổi vai trò</p>
              )}
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">Đặt lại mật khẩu</h3>
