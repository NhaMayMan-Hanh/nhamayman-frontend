...
import { ShoppingBag, Users, Package, FileText, DollarSign, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  const fetchAllData = async (): Promise<void> => {
    try {
      setLoading(true);
      const [productsRes, usersRes, ordersRes, categoriesRes, blogsRes, aboutRes] =
        await Promise.all([
  ...
  const totalStock = stats.products.reduce((sum, p) => sum + (p.stock || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div>
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Dashboard NhaMayMan-Hanh</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            ...
            subtitle={`${stats.users.filter((u) => u.role === "admin").length} admin`}
            color="bg-purple-500"
          />
          <StatCard
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">Sản phẩm theo danh mục</h2>
          ...
          {/* Order Status */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">Trạng thái đơn hàng</h2>
            <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          {/* Category Stats */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">Thống kê danh mục</h2>
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold text-blue-600">
                      {stats.products.filter((p) => p.category === cat.name).length}
                    </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <InfoCard
            ...
            value={`${(
              stats.products.reduce((sum, p) => sum + p.price * p.stock, 0) / 1000000
            ).toFixed(1)}M VNĐ`}
            color="bg-indigo-500"
          />
          <InfoCard
            ...
            title="Admin"
            value={stats.users.filter((u) => u.role === "admin").length.toString()}
            color="bg-pink-500"
          />
        </div>
      </div>
