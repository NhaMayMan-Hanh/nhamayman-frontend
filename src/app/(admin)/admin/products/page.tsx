    ...
    >
      ...
      >
        ...
        <div className="border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h3 className="text-xl font-semibold text-gray-900">Xác nhận xóa sản phẩm</h3>
          ...
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 mt-1">Danh mục: {product.category}</p>
                <p className="text-sm font-medium text-gray-700 mt-2">
                  Giá: {product.price.toLocaleString("vi-VN")}₫
                </p>
                {product.stock === 0 && (
                  <p className="text-xs text-red-600 mt-1 font-medium">Hết hàng</p>
                )}
              </div>
            </div>
          </div>
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/products/${productToDelete._id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
